...
internal class SpecifiedTokenMissing(string target, List<MigxnNode> badTree) : BadMigxnTree
{
    ...
    public static Diagnostic<T> Create<T>(string target, params Span<MigxnNode> spanChildren) where T : class
        => new Diagnostic<T>(new SpecifiedTokenMissing(target, [..spanChildren]));
