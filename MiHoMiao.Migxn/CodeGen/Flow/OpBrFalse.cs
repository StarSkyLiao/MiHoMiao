using MiHoMiao.Migxn.CodeGen.Compare;

namespace MiHoMiao.Migxn.CodeGen.Flow;

internal class OpBrFalse(string labelName) : MigxnOpCode
{
    public override string ToString() => $"{"br.false",-12}{labelName}";
    public override void OnEmitting(List<MigxnOpCode> codes, int index)
    {
        if (index == 0)
        {
            base.OnEmitting(codes, index);
            return;
        }
        MigxnOpCode last = codes[index - 1];
        last = last switch
        {
            OpCgt => new OpBle(labelName),
            OpCge => new OpBlt(labelName),
            OpClt => new OpBge(labelName),
            OpCle => new OpBgt(labelName),
            OpCeq => new OpBne(labelName),
            OpCne => new OpBeq(labelName),
            _ => last,
        };
        if (codes[index - 1] == last) base.OnEmitting(codes, index);
        else codes[index - 1] = last;
    }
}