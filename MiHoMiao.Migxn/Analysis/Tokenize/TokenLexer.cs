...
using MiHoMiao.Migxn.Analysis.Diagnostic;

namespace MiHoMiao.Migxn.Analysis.Tokenize;

public class TokenLexer
{
    private TokenLexer(string input) => m_Input = input;
        
    public static TokenLexer Parse(string input)
    {
        TokenLexer result = new TokenLexer(input);
    ...
    /// 解析到的 Token.
    /// </summary>
    public IList<MigxnToken> MigxnTokens => m_MigxnTokens;
    ...
    private IEnumerable<MigxnToken> Lex()
    {
        ...
        while (Current != '\0')
        {
            IEnumerable<MigxnToken?> matchResult = null;// MigxnToken.TokenLists.Select(func => func(this));
            MigxnToken? result = matchResult.OfType<MigxnToken>().FirstOrDefault();
