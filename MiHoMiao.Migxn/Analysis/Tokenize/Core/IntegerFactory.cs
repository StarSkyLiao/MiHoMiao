...
{
    [GeneratedRegex(@"^0[b|B][01_]*[01]L?", RegexOptions.NonBacktracking)]
    private static partial Regex BinIntegerRegex();
    
    [GeneratedRegex(@"^\d(?:_*\d+)*L?", RegexOptions.NonBacktracking)]
    private static partial Regex DecIntegerRegex();
    
    [GeneratedRegex(@"^0[x|X][\da-fA-F_]*[\da-fA-F]L?", RegexOptions.NonBacktracking)]
