...
public class Fixed64MathTests
{
    ...
    [InlineData(1)]
    [InlineData(Math.PI)]
    [InlineData(4 * Math.PI)]
    public void Sin(double value)
    {
        Assert.Equal(Math.Sin(value), Fixed64Math.Sin(value), 6);
    }
    ...
    {
        Assert.Equal((decimal)Math.Cos(value), Fixed64Math.Cos(value).AsDecimal(), 5);
    }
    ...
    {
        Assert.Equal((decimal)Math.Tan(value), Fixed64Math.Tan(value).AsDecimal(), 5);
    }
