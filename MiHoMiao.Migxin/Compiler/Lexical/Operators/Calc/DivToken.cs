    ...
    : MigxinToken(Keyword.AsMemory(), Index, Position), IKeywordToken, ITokenMatcher
{
    ...
    public static MigxinToken LoadToken(int start, (int Line, int Column) position) => new DivToken(start, position);
    public static HashSet<char> StartChars => [.."/"];
    public static uint Priority => 1;
    public static MigxinToken? TryMatch(MigxinLexer migxinLexer) => IKeywordToken.TryMatch<DivToken>(migxinLexer);
